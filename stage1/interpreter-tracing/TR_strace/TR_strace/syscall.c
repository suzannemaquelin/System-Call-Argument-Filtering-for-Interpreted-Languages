...
syscall_exiting_trace(struct tcb *tcp, struct timespec *ts, int res)
{
	...
	tprintf("%ld\n",tcp->scno);
	/* added by rasoulj@bu.edu */
	const char *name = "trace";
	char shm_name[50];
	char msg[SHM_SIZE];
	struct stat sb;
	
	sprintf(shm_name, "%s_%d", name, tcp->pid);
	...
	if( shm_fd != -1 ) {
		if(fstat(shm_fd, &sb) != -1 ) {
			...
			if ( ptr != MAP_FAILED ) {
				if(sb.st_size > 0)
				{
					if(strcmp(tcp->s_ent->sys_name, "fork") == 0 || strcmp(tcp->s_ent->sys_name, "clone")|| strcmp(tcp->s_ent->sys_name, "vfork") == 0)
					{
						((char*)ptr)[SHM_FORK_FLAG_OFFSET] += 1;
						//memset(ptr+SHM_FORK_FLAG_OFFSET, '1', 1);
					}
					int callstack_len = strlen(ptr);
					if( callstack_len > 0)
					{
						tprintf(" --- php call stack starts ---\n");
						strncpy(msg, ptr, callstack_len-1);
						msg[callstack_len] = '\0';
						tprintf("%s",msg);
						tprintf("\n --- php call stack ends ---\n");
					}
