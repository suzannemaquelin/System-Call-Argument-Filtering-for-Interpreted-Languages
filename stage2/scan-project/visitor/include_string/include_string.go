...
func (st *StringTrie) AddLeaf(leaf StringTrie) {
	// if st.Dynamic == false && st.Content == 0 && len(st.Children) == 0 {
	// 	st.Dynamic = leaf.Dynamic
	// 	st.Content = leaf.Content
	// 	st.Children = leaf.Children
	// 	return
	// }
	leafcopy := StringTrie{}
	...
	if st.Constant != "" {
		if c, ok := (*ct)[st.Constant]; ok {
			if c.IsSimpleString(ct) {
				...
				l.Log(l.Info, "Resolved constant %+v : %s", st.Constant, st.Content)
				st.Constant = ""
			} else {
				l.Log(l.Info, "Could not resolve constant %s : %+v", st.Constant, c)
				c.Constant = ""
				c.Dynamic = true
				// Dynamic constant error
			}
		} else {
			// No such constant Error
		}
	}
	for i, _ := range st.Children {
