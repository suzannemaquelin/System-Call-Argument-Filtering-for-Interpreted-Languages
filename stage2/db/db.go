...
func OpenDb(filepath string) (*DB, error) {
	...
	checkErr(err)
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS function (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT, UNIQUE(name))")
	checkErr(err)
	...
	checkErr(err)
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS syscall_args_requirement (syscall_id INTEGER, function_id INTEGER, syscall_args STRING, FOREIGN KEY(syscall_id) REFERENCES syscall(id), FOREIGN KEY(function_id) REFERENCES function(id))")
	checkErr(err)
...
func (tx *Tx) CreateFunction(name string) error {
	_, err := tx.Exec(`INSERT OR REPLACE INTO function (name) VALUES (?)`, name)
	return err
}
...
func (tx *Tx) CreateSyscallArgsRequirement(function_name string, syscall_name string, syscall_args string) error {
	query := `INSERT OR REPLACE INTO syscall_args_requirement (function_id, syscall_id, syscall_args) SELECT
	(SELECT id FROM function WHERE name=?) as function_id,
	(SELECT id FROM syscall WHERE name=?) as syscall_id,
	? as syscall_args`
	_, err := tx.Exec(query, function_name, syscall_name, syscall_args)
	return err
}
func (tx *Tx) CreateSyscallArgsRequirementFromSyscallNr(function_name string, syscall_number int, syscall_args string) error {
	query := `INSERT OR REPLACE INTO syscall_args_requirement (function_id, syscall_id, syscall_args) SELECT
	(SELECT id FROM function WHERE name=?) as function_id,
	(SELECT id FROM syscall WHERE number=?) as syscall_id,
	? as syscall_args`
	_, err := tx.Exec(query, function_name, syscall_number, syscall_args)
	return err
}
...
func (Db *DB) GetSyscallIdsForFile() map[string][]int {
	var syscalls = make(map[string][]int)
	query := `
WITH RECURSIVE
includes(x, y) AS (
	SELECT * FROM (SELECT INC.file, INC.resolved_include
	FROM include INC 
	union
	SELECT RCI.file as file, RCI.resolved_class_file as resolved_include FROM resolved_class_instance RCI)
	union
	SELECT includes.x, INC.resolved_include 
	FROM includes, 
	(SELECT INC.file, INC.resolved_include
	FROM include INC 
	union
	SELECT RCI.file as file, RCI.resolved_class_file as resolved_include FROM resolved_class_instance RCI) INC
	WHERE INC.file = includes.y AND INC.file != includes.x
)
SELECT DISTINCT I.x, SC.number FROM syscall SC 
INNER JOIN syscall_requirement SCR ON SC.id=SCR.syscall_id 
INNER JOIN function F ON SCR.function_id=F.id 
INNER JOIN function_call L ON L.name=F.name 
INNER JOIN includes I ON L.File=I.x OR L.File=I.y;
`
	...
	for rows.Next() {
		file := ""
		syscall := 0
		err := rows.Scan(&file, &syscall)
		checkErr(err)
		syscalls[file] = append(syscalls[file], syscall)
	}
	return syscalls
}

func (Db *DB) GetSyscallIdsAndArgsForFile() (map[string][]int, map[string]map[int][]string) {
	var syscalls = make(map[string][]int)
	var syscallargs = make(map[string]map[int][]string)
	query := `
WITH RECURSIVE
includes(x, y) AS (
	SELECT * FROM (SELECT INC.file, INC.resolved_include
...
SELECT DISTINCT I.x, SC.number, SCR.syscall_args FROM syscall SC 
INNER JOIN syscall_args_requirement SCR ON SC.id=SCR.syscall_id 
INNER JOIN function F ON SCR.function_id=F.id 
INNER JOIN function_call L ON L.name=F.name 
INNER JOIN includes I ON L.File=I.x OR L.File=I.y;
`
	...
	for rows.Next() {
		...
		syscall := 0
		arg := ""
		err := rows.Scan(&file, &syscall, &arg)
		checkErr(err)
		syscalls[file] = append(syscalls[file], syscall)
		if _, ok := syscallargs[file]; !ok {
			syscallargs[file] = make(map[int][]string)
		}
		syscallargs[file][syscall] = append(syscallargs[file][syscall], arg)
		// fmt.Printf(" %s", arg)
	}
	return syscalls, syscallargs
}

func (Db *DB) GetSyscallNamesForFile() map[string][]string {
	var syscalls = map[string]map[string]struct{}{}
	var result = make(map[string][]string)
	var functions = map[string]map[string]struct{}{}
	query := `
WITH RECURSIVE
includes(x, y) AS (
	SELECT * FROM (SELECT INC.file, INC.resolved_include
...
SELECT DISTINCT I.x, SC.name FROM syscall SC 
INNER JOIN syscall_requirement SCR ON SC.id=SCR.syscall_id 
INNER JOIN function F ON SCR.function_id=F.id 
INNER JOIN function_call L ON L.name=F.name 
INNER JOIN includes I ON L.File=I.x OR L.File=I.y;
...
SELECT DISTINCT I.x, SC.name, L.name FROM syscall SC 
INNER JOIN syscall_requirement SCR ON SC.id=SCR.syscall_id 
INNER JOIN function F ON SCR.function_id=F.id 
INNER JOIN function_call L ON L.name=F.name 
INNER JOIN includes I ON L.File=I.x OR L.File=I.y;
`
	...
	for rows.Next() {
		...
		syscall := ""
		function := ""
		err := rows.Scan(&file, &syscall, &function)
		checkErr(err)
		if _, ok := functions[function]; !ok {
			functions[function] = make(map[string]struct{})
		}
		functions[function][syscall] = struct{}{}
		if _, ok := syscalls[file]; !ok {
			syscalls[file] = make(map[string]struct{})
		}
		syscalls[file][syscall] = struct{}{}
	}
	for k, v := range syscalls {
		for kk := range v {
			result[k] = append(result[k], kk)
		}
	}
	return result
}
...
func (Db *DB) GetDangerousFunctions() []string {
	functions := []string{}
	rows, err := Db.Query(`
	SELECT fn.name FROM function fn, syscall_requirement sc WHERE fn.id == sc.function_id AND sc.syscall_id = 60;
`)
	checkErr(err)
	defer rows.Close()
	for rows.Next() {
